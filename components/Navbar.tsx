...
import { Bell, Briefcase, PlusCircle, Star, UserIcon } from "lucide-react"
        ...
        <div className="max-w-7xl mx-auto px-4 flex justify-between items-center h-full">
          ...
          <div className="hidden md:flex space-x-6 font-medium text-gray-800 justify-center items-center flex-1">
            {mainNavLinks.map(link => (
              <Link key={link.href} href={link.href} className="hover:text-purple-600 transition">
                {link.label}
              </Link>
            ))}
          </div>
          ...
          <div className="flex items-center space-x-4 relative">
            ...
            {status === "unauthenticated" && (
              <div className="hidden md:flex space-x-2">
                <Link href="/auth/login" className="hover:text-purple-600 font-medium transition mt-2 text-[#0d1129]">
                  Login
                </Link>
                <Link
                  href="/joinasprofessional"
                  className="px-4 py-2 bg-purple-500 hover:bg-purple-600 text-white rounded-full font-medium shadow-sm transition"
                >
            ...
            {status === "authenticated" && session.user.role === 'CUSTOMER' && (
              <div className="relative" ref={profileRef}>
                <button
                  ...
                  className="w-11 h-11 rounded-full flex items-center cursor-pointer justify-center text-black hover:bg-purple-50 transition shadow-sm border border-gray-200"
                  aria-haspopup="menu"
                ...
                {profileOpen && (
                  <div className="absolute right-0 mt-3 w-60 bg-white rounded-xl shadow-2xl border border-gray-100 p-2 animate-slide-down">
                    ...
                    <div className="mt-1">
                      <Link href="/customer/my-jobs" className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-gray-700 hover:bg-purple-50 hover:text-purple-600 rounded-lg transition" role="menuitem">
                        <Briefcase className="w-5 h-5" /> My Jobs
                      </Link>
                      <Link href="/customer/post-a-job" className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-gray-700 hover:bg-purple-50 hover:text-purple-600 rounded-lg transition" role="menuitem">
                        <PlusCircle className="w-5 h-5" /> Post a Job
                      </Link>
                      <Link href="/customer/notifications" className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-gray-700 hover:bg-purple-50 hover:text-purple-600 rounded-lg transition" role="menuitem">
                        <Bell className="w-5 h-5" /> Notifications
                      </Link>
                      <button
                        onClick={() => setReviewModalOpen(true)}
                        className="w-full flex items-center gap-3 px-3 py-2.5 text-left text-gray-700 hover:bg-purple-50 hover:text-purple-600 rounded-lg transition cursor-pointer"
                        role="menuitem"
                      >
                        <Star className="w-5 h-5" /> Give Feedback
                      </button>
                    </div>
          ...
          <div className="md:hidden mt-2 mx-4 mb-4 px-4 pb-4 pt-2 space-y-2 bg-white shadow-md rounded-md">
            ...
            {status === "unauthenticated" && (
              <>
                ...
                <Link
                  href="/joinasprofessional"
                  className="block py-2 px-2 text-white bg-purple-500 hover:bg-purple-600 rounded font-medium text-center transition"
                >
                  Join as a Professional
                </Link>
              </>
